...
export class ComplianceAgent {
  ...
  public static evaluatePortfolio(portfolio: Portfolio): ComplianceAlert[] {
    ...
    for (const alloc of allocations) {
      ...
      if (isOver && bestPerformingAsset && bestPerformingAsset.historicalPerformance?.twelveMonths) {
        ...
        if (perf > 5) {
          mandateVsInternalExplanation += `\n\n[Análise de Mercado]: O desenquadramento foi impulsionado majoritariamente por ganhos de capital (valorização passiva de ativos), e não por falha de governança em novos aportes (Dados de benchmark comparativo não disponíveis).`;
        ...
        if (perf < -5) {
          mandateVsInternalExplanation += `\n\n[Análise de Mercado]: O desenquadramento (abaixo do piso) foi causado majoritariamente pela desvalorização dos ativos (perda de capital), não caracterizando resgate manual indevido (Dados de benchmark comparativo não disponíveis).`;
  ...
  public static evaluatePerformance(portfolio: Portfolio, cdi12m: number = 14.63, thresholdPP: number = -2.0): PerformanceAlert[] {
    ...
    portfolio.assets.forEach(asset => {
      if (asset.historicalPerformance && asset.historicalPerformance.twelveMonths !== undefined && asset.historicalPerformance.twelveMonths !== null) {
        const activeReturn = asset.historicalPerformance.twelveMonths;
        const calculatedDeviation = activeReturn - cdi12m;

        if (calculatedDeviation <= thresholdPP) {
          alerts.push({
            id: `perf-${portfolio.id}-${asset.id}-${Date.now()}`,
            portfolioId: portfolio.id,
            portfolioName: portfolio.name,
            assetId: asset.id,
            ticker: asset.ticker,
            ...
            calculatedDeviation,
            thresholdPP,
            message: `Ativo ${asset.ticker} apresenta subperformance. Retorno 12m (${activeReturn.toFixed(2)}%) está ${Math.abs(calculatedDeviation).toFixed(2)} p.p. abaixo do CDI (${cdi12m.toFixed(2)}%).`,
            timestamp
          });
        }
      }
