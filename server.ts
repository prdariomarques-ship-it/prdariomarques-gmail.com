...
} from './src/server/portfolioRepo.ts';
import {
...
app.post('/api/portfolios/enrich-market-data', async (req, res) => {
  try {
    const result = await enrichPortfoliosWithMarketData();
    res.json({
      success: true,
      message: 'Dados de mercado atualizados com sucesso.',
      missingDataAssets: result.missingDataAssets
    });
  } catch (error) {
    console.error('Error enriching market data:', error);
    res.status(500).json({ success: false, error: 'Falha ao enriquecer dados de mercado' });
  }
});
// ==========================================
...
app.get('/api/telemetry', (req, res) => {
  try {
    ...
    for (const port of portfolios) {
      const pAlerts = ComplianceAgent.evaluatePerformance(port, 10.5, -2.0);
      performanceAlerts.push(...pAlerts);
    }
...
async function startServer() {
  ...
  setInterval(() => {
    ...
    enrichPortfoliosWithMarketData().then((res) => {
    console.log('[MarketData] Initialization complete. Missing data for ' + res.missingDataAssets.length + ' assets.');
  });
